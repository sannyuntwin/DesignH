...
export default function KeyboardShortcuts() {
  ...
  const shortcuts: KeyboardShortcut[] = [
    ...
    },
    {
      ...
      ctrlKey: true,
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
    ...
    },
    {
      ...
      action: () => {
        if (currentPageElements.length > 0) {
          const allIds = currentPageElements.map(el => el.id)
          useCanvasStore.getState().selectMultipleElements(allIds)
        }
      },
    ...
    // Quick color shortcuts
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { color: '#000000' }))
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { color: '#ef4444' }))
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { color: '#3b82f6' }))
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { color: '#10b981' }))
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { color: '#f59e0b' }))
      },
    ...
    // Alignment shortcuts
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { textAlign: 'left' }))
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { textAlign: 'center' }))
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { textAlign: 'right' }))
      },
    ...
    // Arrow keys for precise movement
    {
      ...
      action: () => {
        ...
        if (activeIds.length > 0) {
          startMovement()
          activeIds.forEach(id => {
            const el = currentPageElements.find(e => e.id === id)
            if (el) updateElementPosition(id, el.x, el.y - 1)
          })
    ...
    },
    {
      ...
      action: () => {
        ...
        if (activeIds.length > 0) {
          ...
          activeIds.forEach(id => {
            const el = currentPageElements.find(e => e.id === id)
            if (el) updateElementPosition(id, el.x - 1, el.y)
          })
          debouncedSaveHistory()
        }
      },
    ...
    },
    {
      ...
      action: () => {
        ...
        if (activeIds.length > 0) {
          ...
          activeIds.forEach(id => {
            const el = currentPageElements.find(e => e.id === id)
            if (el) updateElementPosition(id, el.x, el.y - 10)
          })
          debouncedSaveHistory()
        }
      },
    ...
    },
    {
      ...
      action: () => {
        ...
        if (activeIds.length > 0) {
          ...
          activeIds.forEach(id => {
            const el = currentPageElements.find(e => e.id === id)
            if (el) updateElementPosition(id, el.x - 10, el.y)
          })
          debouncedSaveHistory()
        }
      },
    ...
    // Layer shortcuts
    {
      ...
      action: () => {
        ...
        activeIds.forEach(id => {
          const element = currentPageElements.find(el => el.id === id)
          if (element) updateElement(id, { zIndex: element.zIndex + 1 })
        })
      },
    ...
    },
    {
      ...
      action: () => {
        ...
        activeIds.forEach(id => {
          const element = currentPageElements.find(el => el.id === id)
          if (element) updateElement(id, { zIndex: Math.max(0, element.zIndex - 1) })
        })
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { zIndex: 999 }))
      },
    ...
    },
    {
      ...
      action: () => {
        const activeIds = selectedElements.length > 0 ? selectedElements : (selectedElement ? [selectedElement] : [])
        activeIds.forEach(id => updateElement(id, { zIndex: 0 }))
      },
...
export function KeyboardShortcutsHelp() {
  const shortcuts = [
    ...
    { keys: 'Ctrl+Tab', description: 'Next element' },
    { keys: 'Ctrl+Shift+Tab', description: 'Previous element' },
    { keys: 'Delete/Backspace', description: 'Delete selection' },
    { keys: 'Ctrl+Delete', description: 'Clear all' },
